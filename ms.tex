...
\caption{\label{fig:pspecs} Left to right from top: power spectra centered on redshifts 10.3,8.5, 7.9, 7.7 and 7.6 (frequencies: 127, 149.5, 159.5 164.5, and 167 MHz.)  All with an effective bandwidth of $\pm$10MHz  covering the redshift span $\Delta z\approx$1421MHz ($B/f^2$) which ranges from 0.8 at $z=$10.3 to 0.45 at $z=$7.4.  The noise curve (blue dashes) is calculated using the method described in \cite{Pober:2014p10390}  and indicates the 1$\sigma$ confidence bounds on data points consisting purely of noise; 65\% of uncorrelated noise like data points will lie below the curve. Note that due to the weighting of the delay transform, adjacent $k$ bins are 50\% correlated. The black line is a fiducial model at 50\% ionization \citep{Lidz:2008p8251}. The redshift 7.68 bin has been reprocessed for comparison with the P14 result. Note that the LST range processed here is 3 hours less than P14 for a small reduction in sensitivity but large reduction in foreground. }
\end{figure}
\section{Results}
\label{sec:results}
